...
public class Testing {
		...
		while(run){
			...
			switch(path){
					...
					for(int counter = 0; counter < events.size(); counter++){
						if(events.get(counter).getEventID() == Integer.valueOf(necessary_info.get(0))){
							for(int counter2 = 0; counter2 < students.size(); counter2++){
								if(students.get(counter2).getEmail().equals(necessary_info.get(1))){
									students.get(counter2).attendEvent(events.get(counter));
									System.out.println("Attendance marked for "+students.get(counter2).getEmail());
									System.out.println();
								}
							}
						}
				...
				case("View Student"):
					...
					System.out.println();
					for(int counter2 = 0; counter2 < students.size(); counter2++){
						if(students.get(counter2).getEmail().equals(path2)){
	...
	public static Event Create_Event(){
		...
		System.out.println("Type \"eventID,eventName,eventLocation,date & time,eventCategory,pointValue\"");
